...
SK_ImGui_LatentSyncConfig (void)
{
  if (config.render.framerate.present_interval == 0)
  {
    ...
    if (config.render.framerate.target_fps > 0.0f)
    {
        ...
        if (bTargetInputInMs)
        {
          if (ImGui::InputFloat ("Target Input Latency", &config.render.framerate.latent_sync.auto_bias_target.ms))
          {
            fLastTargetInputMs = (
              ...
              config.render.framerate.latent_sync.auto_bias_target.ms, 0.0f, 25.0f )
            );
          }
        }
      ...
      ImGui::SetItemTooltip ("Draws color-cycling bars to help locate tearing while VSYNC is off");
      ...
      if (bAdvanced)
      {
        static bool bResyncRateInSeconds =
          ( config.render.framerate.latent_sync.scanline_resync >= 0 );

        if (config.render.framerate.latent_sync.scanline_resync != 0)
        {
          bResyncRateInSeconds =
            config.render.framerate.latent_sync.scanline_resync > 0;
        }

        static int iLastResyncRateFrames  = !bResyncRateInSeconds
          ? config.render.framerate.latent_sync.scanline_resync
          : -750;

        static int iLastResyncRateSeconds =  bResyncRateInSeconds
          ? config.render.framerate.latent_sync.scanline_resync
          : 30000; // Saved in milliseconds

        if (ImGui::Checkbox ("Resync Rate in seconds", &bResyncRateInSeconds))
        {
          config.render.framerate.latent_sync.scanline_resync =
            bResyncRateInSeconds
              ? iLastResyncRateSeconds
              : iLastResyncRateFrames;
        }

        int iResyncRate = bResyncRateInSeconds
          ?  config.render.framerate.latent_sync.scanline_resync / 1000
          : -config.render.framerate.latent_sync.scanline_resync;

        if ( ImGui::SliderInt ( "Resync Rate",
                                  &iResyncRate,
                                    0, 1000, bResyncRateInSeconds
                                              ? "%d Second(s)"
                                              : "%d Frame(s)" ) )
        {
          iResyncRate = std::max (0, iResyncRate);

          if (bResyncRateInSeconds)
          {
            iLastResyncRateSeconds =
                iResyncRate        =
                iResyncRate * 1000;
...
SK::Framerate::Limiter::wait (void)
{
  ...
  if (next_ > 0LL)
  {
    ...
    {
      static constexpr int _MIN_FRAMES =  6,
                           _MAX_FRAMES = 30;

      struct {
        double input_min [_MIN_FRAMES] = { },
               input_max [_MAX_FRAMES] = { };

        int frames = 0;

        double getMinInput (void) noexcept
        {
          double avg     = 0.0,
                 samples = 0.0;

          for ( int i = 0; i < std::min (frames, _MIN_FRAMES); ++i )
          {
            ++samples; avg += input_min [i];
          }
        ...
        double getMaxInput (void) noexcept
        {
          double avg     = 0.0,
                 samples = 0.0;
      ...
      auto _RevertACTION = [&](bool bFull = false) -> void
      {
        if (bFull)
        {
          iACTION =
      ...
      if (iTearingMode == SK_TearingMode::AppControlled)
      {
        if (bIsD3D9)
        {
          static DWORD
          ...
          auto _IsTearingD3D9 = [&]() -> bool
          {
            ...
            dwLastTime = SK_timeGetTime () - dwDiff;
      ...
      switch (iTearingMode)
      {
        ...
        // No tearing, reset limiter if variation is too high or input latency is stuck at (-0.1, 0.1) ms
        case SK_TearingMode::AlwaysOff:
        {
          ...
          {
            static bool bWasUnstableFPS = false;

            static DWORD
              dwLastTime = 0;

            bIsUnstableFPS |=
              latency_avg.getMinInput () < 0.0;
            ...
            if (bWasUnstableFPS)
            {
              ...
              else if (! bIsUnstableFPS)
              {
                bWasUnstableFPS = false;
              }
            }
          }
          ...
          if (! bIsComposedPresentMode)
          {
            ...
            bool bHighVariation = [&]() -> bool
            {
              bool bIgnoreHighVariation =
                bIsTearingModeAdaptiveOn ||
                bIsTearingModeAlwaysOff  || // TODO: Remove to allow limiter reset for "Always Off" mode
                bIsUnstableFPS           ||
                bIsVRR;
              ...
              if (! SK_RenderBackend_V2::latency.stale)
              {
                ...
                else
                {
                  ...
                  if (! bIsHighRenderLatency)
                  {
                    ...
                    // Render Latency must be different every frame in 2x.. non-tearing mode
                    // Otherwise, framepacing gets worse for some reason :D
                    static UINT        iLastRenderLatency = iRenderLatency;
                    if (std::exchange (iLastRenderLatency,  iRenderLatency) ==
                                                            iRenderLatency)
                    {
                      if (! (bRecoveredIndependentFlip || dwWaitTime < dwDiff))
                      {
                        return true;
                      }
                    }

                    if (dwWaitTime >= dwDiff)
                    {
                      if (iRenderLatency < iTargetRenderLatency)
                      {
                        if (dwWaitTime >= dwDiff + 500)
                        {
                          dwLastTime = SK_timeGetTime ();
                          dwWaitTime = 0;
                        }
            ...
            bool bFrameBecameStable = [&]() -> bool
            {
              ...
              if (std::exchange (bWasUnstableFPS,  bIsUnstableFPS) &&
                                                  !bIsUnstableFPS)
              {
                return true;
              }
              ...
              return iACTION == ACTION_FrameBecameStable;
            }();
            ...
            if (! bIsAboveRefresh)
            {
              switch (iACTION)
              {
                case ACTION_StuckInputLatency:
                  iACTIONS [1] = ACTION_StuckInputLatency;
                  iACTIONS [2] = ACTION_HighRenderLatency;
                  iACTIONS [3] = ACTION_HighVariation;
                  break;
                case ACTION_HighVariation:
                  iACTIONS [1] = ACTION_HighVariation;
                  iACTIONS [2] = ACTION_HighRenderLatency;
                  iACTIONS [3] = ACTION_StuckInputLatency;
                  break;
                default:
                  break;
              }
            ...
            else if ( ( bIsTearingModeAdaptiveOff &&
                        bIsPreRenderLimit1         ) ||
                      ( iACTION ==
                         ACTION_HighVariation      ) )
            {
              iACTIONS [2] = ACTION_HighVariation;
              iACTIONS [3] = ACTION_StuckInputLatency;
            }

            for ( const auto& vACTION : iACTIONS )
            {
              DWORD dwMaxWaitTime = 1500;
              DWORD    dwWaitTime =
                SK_timeGetTime () - static_cast <DWORD> (std::abs (iLastWaitTime));
              ...
              switch (vACTION)
              {
                case ACTION_HighVariation:
                {
                  ...
                  if (iACTION == ACTION_HighVariation)
                  {
                    ...
                    if (iMultiplier >= 1)
                    {
                      dwMaxWaitTime = iMultiplier * 2000;
                    }
                      ...
                      else if (bIsAboveRefresh)
                      {
                        if (bDontTear)
                        {
                          if ( dwWaitTime >=
                               dwMaxWaitTime )
                          {
                            bHighRenderLatency = false;
                          }
                        }

                        else if (dwWaitTime < 75)
                        {
                          bHighRenderLatency = true;
                        }
                      }
                    ...
                    if (! bIsNewACTION)
                    {
                      ...
                      if (! bStopWait)
                      {
                        if (iLastWaitTime < 0)
                        {
                          if (dwWaitTime >= 750)
                          {
                            if (latency_avg.getMaxInput () < 0.0)
                            {
                              bIsNewACTION = true;
                            }
    ...
    if (config.render.framerate.present_interval == 0 && ticks_per_scanline > 1)
    {
      ...
      bStopFrameSkip |= (! SK_LatentSync_AllowFrameSkip ());

      if (__SK_LatentSyncSkip < 2 || bStopFrameSkip)
  ...
  if (config.render.framerate.present_interval == 0 && ticks_per_scanline > 1)
  {
    ...
    if (config.render.framerate.latent_sync.scanline_resync != 0)
    {
      ...
      bool bResyncInMilliseconds =
        config.render.framerate.latent_sync.scanline_resync > 0;

      if (bResyncInMilliseconds)
      {
        iSync =
          SK_timeGetTime ()
            - dwLastTime;
      ...
      if ( std::round (fps / rb.getActiveRefreshRate ()) >= 2.0 ||
                             rb.gsync_state.active              )
      {
        iResync *= bResyncInMilliseconds ? 10 : 50;
      }

      static bool        bWasResyncInMilliseconds = bResyncInMilliseconds;
      if (std::exchange (bWasResyncInMilliseconds,  bResyncInMilliseconds) !=
                                                    bResyncInMilliseconds)
      {
        iSync = iResync;
      }
